...
Dialog.addMessage("Applies the cell masks saved by the microglia segmentation\n" +
                   "pipeline to other channels.");
Dialog.addString("Channels to quantify (comma-separated ID, e.g. w1,w2,w3)", "w3");
...
Dialog.addCheckbox("Classify cells channel with LCD Fibrils", true);
Dialog.addString("LCD channel's ID (e.g. w2)", "");
...
// Picks the measurement matching the classification statistic.
