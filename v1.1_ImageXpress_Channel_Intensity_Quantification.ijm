// v1.1 Channel Intensity Quantification
// Companion pipeline to v1_5_ImageXpress_Microglia_Segment_Analysis.ijm.
//
//
// This pipeline reads that mask_index, opens the same crop rectangle in
// one or more other channel images (w0, w1, w3, w4) of the same field of
// view, drops the saved mask onto it, and measures intensity (mean, min,
// max, median, stddev, integrated density) inside the mask. This is how
// you quantify a marker's signal within the already-segmented microglia.
//
// v1.1 adds:
//   - mask_overlay_examples/example_overlay_<channel>.png : one example
//     per requested channel showing the mask outline drawn on that
//     channel's crop.
//   - Optional seeded/unseeded classification: pick one channel (e.g. an
//     LCD Fibrils stain) and a statistic + threshold, and every cell gets
//     a seeded_status column based on whether that channel's value for
//     that cell is above or below your threshold.
//
// Requirements / assumptions:
//   - The other-channel files live in one folder and are named the same
//     way as the source (segmentation-channel) image
//   - Each mask_image tif is exactly the same pixel width/height as the
//     crop rectangle (crop_w x crop_h) recorded in mask_index - this is
//     guaranteed automatically if the masks came from v1_5's export.

var masks_folder, channel_input_folder, output_folder, results_csv_path, overlay_folder;
var channel_list, missing_log, overlay_done_channels;
...
Dialog.addCheckbox("Save an example mask-overlay image per channel (visual QC)", true);
...
overlay_done_channels = newArray();
-
...
for (f = 0; f < nidx; f++) {
    ...
    for (li = 1; li < lines.length; li++) {   // li=0 is the header row, skip it
        ...
        // classify this cell as seeded/unseeded from one channel
        cell_lcd_value = NaN;
        cell_seeded_status = "";
        if (do_lcd_classification) {
        ...
        for (c = 0; c < channel_list.length; c++) {
            ...
            // save one example overlay per channel, for a visual sanity check
            if (save_overlay_examples && !arrayContains(overlay_done_channels, target_token)) {
                run("Select None");
                ...
                saveAs("PNG", overlay_folder + "example_overlay_" + target_token + ".png");
                close();
                selectWindow(chan_title);
                overlay_done_channels[overlay_done_channels.length] = target_token;
            }
